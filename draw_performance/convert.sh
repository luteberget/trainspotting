...
for i in models/*railml generated/*railml; do
	...
	t1=$(date +%s.%N)
	./timeout.sh -t 300 ../target/release/vis_rs convert/in/$filename.in -z convert/levelsout/$filename.tikz > /dev/null
	t2=$(date +%s.%N)
	...
	t1=$(date +%s.%N)
	./timeout.sh -t 300 ../rail-layout-gridbased/frontperf hwb $(cat maxheight/$filename) convert/in/$filename.in > convert/gridout/$filename.tikz
	t2=$(date +%s.%N)
	thwb=$(echo "$t2 - $t1" | bc -l)

	t1=$(date +%s.%N)
	./timeout.sh -t 300 ../rail-layout-gridbased/frontperf hbw $(cat maxheight/$filename) convert/in/$filename.in > convert/gridout/$filename.tikz
	t2=$(date +%s.%N)
	thbw=$(echo "$t2 - $t1" | bc -l)

	t1=$(date +%s.%N)
	./timeout.sh -t 300 ../rail-layout-gridbased/frontperf bhw $(cat maxheight/$filename) convert/in/$filename.in > convert/gridout/$filename.tikz
	t2=$(date +%s.%N)
	...
	t1=$(date +%s.%N)
	./timeout.sh -t 300 ../gridvis/gridjson convert/gridvisformat/$filename.g > /dev/null
	t2=$(date +%s.%N)
