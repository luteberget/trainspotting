...
fn plan_accel_brake_intersection(start :Point, restriction :DistanceVelocity, acc: f64, brk: f64) -> (Point, Point) {
    let intersection_dx = (2.0 * brk * restriction.dx + 
...
fn plan_coast_brake_intersection(start :Point, restriction :DistanceVelocity, brk :f64) -> (Point, Point) {
    let brake_dx = (start.v * start.v - restriction.v * restriction.v) / (2.0 * brk);
    let brake_dt = (start.v - restriction.v) / brk;
...
pub fn dynamic_plan_step(train :&TrainParams, max_dist :f64,
    ...
    let p = Point { t: 0.0, x: 0.0, v: current_velocity };
    ...
    for &restriction in profile.max_velocity_ahead.iter() {
        ...
        let (acc,brk) = plan_accel_brake_intersection(p, restriction, 
                                                      train.max_acc, train.max_brk);
        accel_plans.push(acc);
        brake_plans.push(brk);

        // Coasting limited by braking curve (+ braking curve)
        let (coast, brk) = plan_coast_brake_intersection(p, restriction,
                                                         train.max_brk);
    ...
    if shortest_accel_plan.t > tol {
        DriverPlan { action: DriverAction::Accel, dt: shortest_accel_plan.t }
    } else if shortest_coast_plan.t > tol {
        DriverPlan { action: DriverAction::Coast, dt: shortest_coast_plan.t }
    } else {
        DriverPlan { action: DriverAction::Brake, dt: shortest_brake_plan.t }
    }
}
